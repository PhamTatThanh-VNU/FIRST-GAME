...
void Character::HandleEvent(SDL_Event& e, Mix_Chunk* gJump, Mix_Chunk* gTele)
{
	...
	if (e.type == SDL_KEYDOWN && e.key.repeat == 0)
	{
		...
		case SDLK_UP:
		{
			if (!cEvent1 && (currentTime - jumpStartTime) >= 300) {
		...
		case SDLK_LSHIFT:
		{
			Uint32 currentTimeShift = SDL_GetTicks();
			if (currentTimeShift - lastShiftTime >= 10000) {
				cEvent = true;
				lastShiftTime = currentTimeShift;
				Mix_PlayChannel(MIX_CHANNEL, gTele, NOT_REPEATITIVE);
			}
			// Add a 5 second timer
			SDL_AddTimer(5000, TimerCallback, &cEvent);
			break;
		}
		}
	}
...
bool Character::check_collision_clips(SDL_Rect Threat,SDL_Rect Threat1,SDL_Rect Threat2 , SDL_Rect* clips_threat, SDL_Rect* clips_threat1, SDL_Rect* clips_threat2, SDL_Rect* currentClips_character)
{
	...
	// check collision OBS
	if (right_player - TRASH_CHARACTER_RIGHT >= left_threat + 20 && left_player + TRASH_CHARACTER_LEFT + 10 <= right_threat - TRASH_OBS_RIGHT)
	{
	...
	if (right_player - TRASH_CHARACTER_RIGHT >= left_threat2 + 20 && left_player + TRASH_CHARACTER_LEFT <= right_threat2 )
	{
...
Uint32 Character::TimerCallback(Uint32 interval, void* param)
{
	bool* cEvent = static_cast<bool*>(param);
	*cEvent = false;
	return 0;
}
...
void Character::DrawTextShift(LTexture gTextTexture,
	SDL_Color textColor,
	...
	if ((currentTime - lastShiftTime) >= 10000 && cEvent == false) { // Kiểm tra thời gian trôi qua và tính năng dịch chuyển đã sẵn sàng
		if (gTextTexture.LoadFromFileTTF(path, gFont, textColor, gRenderer)) {
			gTextTexture.Render(TEXTSHIFT_POSX, TEXTSHIFT_POSY, gRenderer);
		}
		else {
			gTextTexture.Free();
		}
		...
		gTextTexture.Free(); // Nếu chưa đủ điều kiện, giải phóng bộ nhớ và không hiển thị gTextTexture
	}
}
