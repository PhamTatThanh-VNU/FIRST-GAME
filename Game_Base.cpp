...
SDL_Color textColorSuggest = { 220,20,60 };
TTF_Font* gFont = nullptr;
TTF_Font* gFontSuggest = nullptr; 
Mix_Chunk* gJump = nullptr;
Mix_Music* gLose = nullptr;
		...
		while (!Quit)
		{
			if (Game_State)
			{
				...
				while (SDL_PollEvent(&e) != 0)
				{
					...
					HandlePauseButton(&e, gRenderer, gContinueButton,ContinueButton,gContinueButtonTexture, Game_State, gClick);
				...
				SDL_Rect* currentClipContinue = &gContinueButton[0];
				ContinueButton.Render(currentClipContinue, gRenderer, gContinueButtonTexture);
				// Check for the P key press event
				if (e.type == SDL_KEYUP && (e.key.keysym.sym == SDLK_p)) {
					gContinueButtonTexture.Free();
					// Set the current clip to nullptr to prevent it from being rendered
					currentClipContinue = nullptr;
				}
				...
				character.DrawTextShift(gText3Texture,textColorSuggest,gRenderer,gFontSuggest,"Press and Hold Shift");
				//Check_collision
				if (character.check_collision(Rocket, currentClip_Character) == true || character.check_collision_clips(Threat1, Threat3, Threat2, currentclip_Obstacles, currentClip_Dog, currentclip_FlyOBS, currentClip_Character) == false) {
					Mix_PlayMusic(gLose, NOT_REPEATITIVE);
					Mix_PausedMusic();
...
bool LoadMedia()
{
	...
	gLose = Mix_LoadMUS("sounds/gLose.mp3");
	if (gLose == nullptr) {
		GetError("Failed to load lose sound", MIX_ERROR);
		success = false;
	}
	//Load TEXT
	...
	gFontSuggest = TTF_OpenFont("font/font suggest.ttf", 33);
	if (gFontSuggest == NULL)
	{
		GetError("Failed to load font", MIX_ERROR);
		success = false;
	}
	...
	if (!gContinueButtonTexture.LoadFromFile("imgs/button/big_button/continueandpause.png", gRenderer))
	{
	...
	{
		gContinueButton[0] = {0,0,75,65};
		gContinueButton[1] = {71,0,75,65};
	}
	...
	if (!gFly_Obstacle.LoadFromFile("imgs/Threats/Drop.png", gRenderer)) {
		...
		for (int i = 0; i < 6; ++i) {
			Fly_gObstacle[i] = { 157 * i,0,157,157 };
		}
	}
